...
<ul wire:loading.delay.long.remove wire:target="search" class="p-2 grid w-full spacey-y-2">
    ...
    $authParticipant = $conversation->participant($this->auth);
    $receiver = $conversation->isGroup() ? null : ($conversation->isPrivate() ? $conversation->peerParticipant($this->auth)?->participantable : $this->auth);
    ...
    $isReadByAuth = $conversation?->readBy($authParticipant ?? $this->auth) || $selectedConversationId == $conversation->id;
